				...
				for(int i=0;i<boggle.length;i++)
					for(int j=0;j<boggle[0].length;j++)
						if(boggle[i][j]==s.charAt(0))
							if(dfs(s,boggle,i,j,1))
								break outer;
		}
	}

	private static boolean  dfs(String s, char[][] boggle,int i,int j,int pos) {
		...
		for(int d=0;d<dx.length;d++){
			...
			if(ii<0 || ii>=boggle.length || jj<0 || jj>=boggle[0].length)
				continue;
			if(boggle[ii][jj]==s.charAt(pos))
				dfs(s,boggle,ii,jj,pos+1);
		}
